...
router.get('/updatePromotion', checkAuthorization, (req, res) => {
  ...
  var config = {
    method: 'get',
    maxBodyLength: Infinity,
    url: 'https://lapladvote-default-rtdb.firebaseio.com/promoted.json?orderBy="status"&equalTo=0',
    headers: { }
  };
    ...
    Object.entries(result).forEach((entry) => {
      ...
      axios
        ...
        .then(response => {
          ...
          axios.patch(`https://lapladvote-default-rtdb.firebaseio.com/promoted/${key}.json`, {
            status: 1
          })
            .then(response => {
              console.log(response.data);
...
router.get('/updateCoinData', checkAuthorization, (req, res) => {
  ...
  var config = {
    method: 'get',
    maxBodyLength: Infinity,
    url: 'https://lapladvote-default-rtdb.firebaseio.com/coins.json?orderBy="ownerId"&equalTo="sKsvOQnlk1WYfTuiYgdppT0nusH3"',
    headers: { }
  };
  ...
  .then(function (response) {
    ...
    Object.entries(result).forEach((entry) => {
      ...
      .then((response) => {
        ...
        axios.patch(`https://lapladvote-default-rtdb.firebaseio.com/coins/${key}.json`, {
...
router.get('/verifyIp/:ip', async (req, res, next) => {
	try {
		...
		const response = await axios.get(
			`https://lapladvote-default-rtdb.firebaseio.com/ipaddresses/.json`
		);
		const result = response.data;
		if (result) {
			var keys = [];
		...
		if (!id) {
			...
			if (Date.now() - data[point].time >= 3600000 || data[point].n < 2) {
				if (data[point].n < 2) {
					await axios.patch(
						`https://lapladvote-default-rtdb.firebaseio.com/ipaddresses/${id}.json`,
						{
							ip: ip,
							n: 2,
						}
